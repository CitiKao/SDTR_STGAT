...
$RunDir = Join-Path $Repo "runs\rcog_safety_refined_$Stamp"
New-Item -ItemType Directory -Force -Path $RunDir | Out-Null
$Log = Join-Path $RunDir "terminal.log"
$Summary = Join-Path $RunDir "rcog_safety_refined_summary.json"
Set-Content -Path (Join-Path $Repo "runs\latest_rcog_safety_refined_run.txt") -Value $RunDir

Write-Host "========================================================================================================" -ForegroundColor Cyan
Write-Host "RCOG safety refined search started" -ForegroundColor Cyan
...
$PythonArgs = @(
  ...
  "--caps", "0.26,0.28,0.30,0.32,0.34,0.36",
  "--max-activations", "8,9,10,11,12",
  "--min-precisions", "58,60,62,64,66",
  "--bad-weights", "1.0,1.5,2.0",
  "--objectives", "ratio,precision,balanced",
  "--bootstrap-top-k", "20",
...
if ($ExitCode -eq 0) {
  ...
  Write-Host "RCOG safety refined search finished successfully." -ForegroundColor Green
  Write-Host "Summary: $Summary" -ForegroundColor Green
  Write-Host "========================================================================================================" -ForegroundColor Green
} else {
  Write-Host "========================================================================================================" -ForegroundColor Red
  Write-Host "RCOG safety refined search failed with exit code $ExitCode." -ForegroundColor Red
